var Slack = require("slack-client"),
	log = require("../log.js"),
	config = require("../../config/config.js"),
	EventEmitter = require("events").EventEmitter;

exports.event = new EventEmitter();

var slackClient = new Slack(config.slack_key, true, true);

slackClient.on("open", function() {
	log.log("Connected to Slack as " + slackClient.self.name);
});

slackClient.login();

slackClient.on("message", function(msg) {
	if (msg.text == null)
		return;
	exports.event.emit("msg", slackClient.getUserByID(msg.user), msg.channel, msg.text);
});

slackClient.on("userChange", function(user) {
    exports.event.emit("join", user);
});

slackClient.on("presenceChange", function(data) {
	exports.event.emit("presence", slackClient, data.user, data.presence);
});

exports.say = function(to, msg) {
	var channel = slackClient.getChannelGroupOrDMByID(to);

	channel.send(msg);
}

exports.client = slackClient;